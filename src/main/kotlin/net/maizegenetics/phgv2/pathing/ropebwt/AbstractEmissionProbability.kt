...
abstract class AbstractEmissionProbability(val readMap: Map<Int, MutableList<Ps4gGameteSet>>,
     ...
     */
    fun getHaploidEmissionProbabilityArray(positionIndex: Int) : DoubleArray {
        ...
        if (numberOfReads > maxNumberOfTrials) {
            ...
            for (parentIndex in parentList) {
                ...
                probabilityArray[arrayPtr++] = binomialProbability(distribution, indexClassCounts)
            }
        } else {
            for (parentIndex in parentList) {
                val indexClassCounts = indexCountsForOneIndex(gameteSetList, parentIndex)
            ...
            for (parent1 in parentList) {
                for (parent2 in parentList) {
                    probabilityArray[arrayPtr++] = if (parent1 == parent2) {
                        ...
                        binomialProbability(distribution, indexClassCounts)
                    } else {
    ...
    fun cacheSomeProbabilities(): Array<DoubleArray> {
        val binomialProbabilities = Array(maxNumberOfTrials + 1) {i -> DoubleArray(i + 1)}
        binomialProbabilities[0][0] = 0.0
        for (numberOfTrials in 1 .. maxNumberOfTrials) {
            ...
            for (numberOfSuccesses in 0 .. numberOfTrials)
                binomialProbabilities[numberOfTrials][numberOfSuccesses] = binomialProbability(distribution, numberOfSuccesses)
        }
        return binomialProbabilities
    }

    abstract fun binomialProbability(distribution: BinomialDistribution, numberOfSuccesses: Int): Double
