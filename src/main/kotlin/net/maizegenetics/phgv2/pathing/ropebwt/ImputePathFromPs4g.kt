...
class ImputePathFromPs4g: CliktCommand(help = "Impute best haplotypes from a Ps4g file.")   {
    ...
    val outPathDir by option(help="The directory where the imputed assembly haplotypes will be written for each sample. " +
            "File names will be <sampleName>_imputed_path.txt. If no directory name is supplied, the files will not be written.")
        .required()

    val pathType by option(help = "The type of path to find. Must be lower case 'haploid' or 'diploid' (without quotes). " +
    ...
    val probSame by option(help = "The probability that a path stays on the same gamete when transitioning between " +
