...
class ViterbiHMM(val inbreedingCoefficient: Double, val sameGameteProbability: Double, val probCorrect: Double
    ...
    fun findDiploidPath(contig: String,
        ...
        val emissionProbabilityCalculator = DiploidEmissionProbabilityForViterbiHMM(readMap, likelyParentSet, probCorrect)
        val emissionP = emissionProbabilityCalculator::getLnEmissionProbabilityArray
    ...
    /**
     * Launches one coroutine per time step to compute the full emission vector
     * (one probability per state). Work is spread across [parallelism] worker
     * threads via a limited slice of Dispatchers.Default.
     *
     * The HMM recursion consumes results with `deferredEmissions[t].await()`.
     * Because workers race ahead of the sequential consumer, await() essentially
     * never suspends in practice -- the main thread doesn't wait.
     */
//    fun CoroutineScope.precomputeEmissions(
//        numObservations: Int,
//        numStates: Int,
//        model: EmissionModel,
//        parallelism: Int = Runtime.getRuntime().availableProcessors()
//    ): List<Deferred<DoubleArray>> {
//        val workerDispatcher = Dispatchers.Default.limitedParallelism(parallelism)
//        return (0 until numObservations).map { t ->
//            async(workerDispatcher) {
//                DoubleArray(numStates) { s -> model.probability(s, t) }
//            }
//        }
//    }
