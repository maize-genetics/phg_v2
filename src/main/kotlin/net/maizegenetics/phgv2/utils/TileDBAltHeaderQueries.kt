...
class TileDBAltHeaderQueries {
    companion object {
        private val myLogger = LogManager.getLogger(TileDBAltHeaderQueries::class.java)
        ...
        fun queryWithStreaming_sampleNameIdByRefRange(arrayName: String, refRangeList: List<String>): Map<String,List<Map<String,String>>> {
            ...
            myLogger.info("queryWithStreaming: refRangeList size = ${refRangeList.size},  refRangeBuffers.first size = ${refRangeBuffers.first.size}, second size = ${refRangeBuffers.second.size}")
            // print the refRangeBuffers.second as a long array
            val refRangeOffsetsArray = refRangeBuffers.second.toJavaArray() as LongArray
            myLogger.info("queryWithStreaming: refRangeOffsetsArray = ${refRangeOffsetsArray.contentToString()}")
            ...
            val idBuffer = NativeArray(context, idBufferSize, Datatype.TILEDB_STRING_ASCII)
            val idOffsets = NativeArray(context, idOffsetsSize, Datatype.TILEDB_UINT64)
        ...
        //    }
            ...
            do {
                ...
                myLogger.info("queryWithStreaming the query.submit results show : num SampleNames=$numSampleNames, num IDs=$numIDs, num RefRanges=$numRefRanges")
            ...
            myLogger.info("queryWithStreaming: number of  resultMap entries = ${resultMap.keys.size}, num filtered results = ${filteredResults.keys.size}")
            //myLogger.info("\nHere are all the values from the resultMap before filtering:")
            resultMap.forEach { (key, value) -> myLogger.info("Key: $key, Value: $value") }
