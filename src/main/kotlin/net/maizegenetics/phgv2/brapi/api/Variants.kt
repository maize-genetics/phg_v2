...
fun Route.variants() {
    route("/variants") {
        ...
        get("") {
            val pageToken =
                call.parameters["page"]?.toInt()
                    ?: 1 // token is index into the list of reference ranges, which are stored in sorted order
            ...
            if (variant == null) {
                ...
                call.respond(
                    VariantSingleResponse(
                        MetadataTokenPagination(pagination = TokenPagination(currentPageToken = "1")),
                        variant
                    )
                )
            }
