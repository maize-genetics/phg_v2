...
//        val ranges = bedfileToSRangeSet(bedFileName,referenceFileName)
        ...
        File(mafDirName).walk().filter { !it.isHidden && !it.isDirectory }
            ...
            .forEach {
                ...
                val gvcfVariants = getGVCFVariantsFromMafFile(refGenomeSequence, it.absolutePath, it.nameWithoutExtension, twoGvcfs=twoGvcfs, legacyMafFile=legacyMafFile)
                ...
                if (gvcfVariants.size == 1){
                    ...
                    bgzipAndIndexGVCFfile("${outputDirName}/${it.nameWithoutExtension}.g.vcf", condaEnvPrefix, condaEnvNeeded)
                    ...
                    val hvcfVariants = convertGVCFToHVCF(dbPath,sampleName, ranges, variants, refGenomeSequence, dbPath, asmHeaderLines,condaEnvPrefix,condaEnvNeeded)
                    ...
                    //bgzip the files
                    bgzipAndIndexGVCFfile("${outputDirName}/${it.nameWithoutExtension}.h.vcf", condaEnvPrefix, condaEnvNeeded)
                    ...
                    gvcfVariants.entries.forEachIndexed { index, (name, variants) ->
                        ...
                        bgzipAndIndexGVCFfile(outputNames[index], condaEnvPrefix, condaEnvNeeded)
                        val asmHeaderLines = mutableMapOf<String,VCFHeaderLine>()
                        //convert the GVCF records into hvcf records
                        val hvcfVariants = convertGVCFToHVCF(dbPath,sampleName, ranges, sortedVariants, refGenomeSequence, dbPath, asmHeaderLines,condaEnvPrefix,condaEnvNeeded)
                        val asmHeaderSet = asmHeaderLines.values.toSet()
                        //export the hvcfRecords
                        exportVariantContext(sampleName, hvcfVariants, "${outputDirName}/${it.nameWithoutExtension}.h.vcf",refGenomeSequence, asmHeaderSet)
                        //bgzip the files
    ...
    fun convertGVCFToHVCF(dbPath: String,sampleName: String, bedRanges : List<Pair<Position,Position>>, gvcfVariants: List<VariantContext>,
                          refGenomeSequence : Map<String, NucSeq>, agcArchiveName: String, asmHeaders: MutableMap<String,VCFHeaderLine>,condaEnvPrefix:String = "",condaEnvNeeded:Boolean) : List<VariantContext> {
        ...
        return gvcfVariantsByContig.keys
            ...
            .flatMap { convertGVCFToHVCFForChrom(dbPath, sampleName, bedRegionsByContig[it]!!, refGenomeSequence, agcArchiveName, gvcfVariantsByContig[it]!!, asmHeaders,condaEnvPrefix,condaEnvNeeded) }
    }

    fun convertGVCFToHVCFForChrom(dbPath: String, sampleName: String, bedRanges: List<Pair<Position,Position>>, refGenomeSequence: Map<String, NucSeq>, agcArchiveName: String, variantContexts: List<VariantContext>, asmHeaders: MutableMap<String,VCFHeaderLine>,condaEnvPrefix:String = "",condaEnvNeeded:Boolean ) : List<VariantContext> {
        ...
        val metaDataWithSequence = addSequencesToMetaData(dbPath, outputVariantMetadata, condaEnvPrefix,condaEnvNeeded)
     ...
     */
    fun addSequencesToMetaData(dbPath: String, metadata: List<HVCFRecordMetadata>, condaEnvPrefix:String="",condaEnvNeeded:Boolean) : List<HVCFRecordMetadata> {
