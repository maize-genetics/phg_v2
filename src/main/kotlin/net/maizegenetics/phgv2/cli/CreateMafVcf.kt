...
class CreateMafVcf : CliktCommand(help = "Create g.vcf and h.vcf files from AnchorWave MAF files") {
        ...
        File(mafDirName).walk().filter { !it.isHidden && !it.isDirectory }
            ...
            .forEach { originalFileName ->
                ...
                if (gvcfVariants.size == 1){
                    ...
                    processGameteGVCFToHVCF(gvcfVariants.values.first(), mafToGVCFObject, sampleName, sampleName, arrayOf("${outputDirName}/${originalFileName.nameWithoutExtension}.g.vcf"), 0, refGenomeSequence, dbPath, ranges, outputDirName, originalFileName, numRangesPerAgcQuery)
    ...
    fun convertGVCFToHVCFVariantInfo(dbPath: String,sampleName: String, bedRanges : List<Pair<Position,Position>>, gvcfVariants: List<AssemblyVariantInfo>,
                          refGenomeSequence : Map<String, NucSeq>, asmHeaders: MutableMap<String,VCFHeaderLine>,condaEnvPrefix:String = "", numRangesPerAgcQuery:Int = -1) : List<VariantContext> {
        ...
        return gvcfVariantsByContig.keys
            ...
            .flatMap { convertGVCFToHVCFForChromVariantInfo(dbPath, sampleName, bedRegionsByContig[it]!!, refGenomeSequence, gvcfVariantsByContig[it]!!, asmHeaders,condaEnvPrefix, numRangesPerAgcQuery) }
    }


    private fun convertGVCFToHVCFForChromVariantInfo(dbPath: String, sampleName: String, bedRanges: List<Pair<Position,Position>>, refGenomeSequence: Map<String, NucSeq>, variantInfos: List<AssemblyVariantInfo>, asmHeaders: MutableMap<String,VCFHeaderLine>, condaEnvPrefix:String = "", numRangesPerAgcQuery:Int = -1 ) : List<VariantContext> {
    ...
    override fun run() {
        ...
        if(metricsFile != "") {
            ...
            createASMHvcfs(dbPath, bed, referenceFile, mafDir, outputDir, metricsFile, skipMetrics, false,legacyMafFile, numRangesPerAgcQuery)
        } else {
            createASMHvcfs(dbPath, bed, referenceFile, mafDir, outputDir, "$outputDir/VCFMetrics.tsv", skipMetrics, false,legacyMafFile, numRangesPerAgcQuery)
        }
